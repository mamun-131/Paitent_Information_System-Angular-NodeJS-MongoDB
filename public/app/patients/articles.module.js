System.register(["@angular/core", "@angular/common", "@angular/forms", "@angular/router", "./articles.routes", "./articles.component", "./create_alert/create.component", "./create_symptom/create.component", "./list/list.component", "./list_alert/list.component", "./view/view.component", "./edit/edit.component", "./viewByCourse/view.component", "./viewByStudent/view.component", "./create/create.component", "./view_symptom/view.component", "./view_video/view.component", "./list_tip/list.component", "./create_tip/create.component"], function (exports_1, context_1) {
    "use strict";
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __moduleName = context_1 && context_1.id;
    var core_1, common_1, forms_1, router_1, articles_routes_1, articles_component_1, create_component_1, create_component_2, list_component_1, list_component_2, view_component_1, edit_component_1, view_component_2, view_component_3, create_component_3, view_component_4, view_component_5, list_component_3, create_component_4, ArticlesModule;
    return {
        setters: [
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (common_1_1) {
                common_1 = common_1_1;
            },
            function (forms_1_1) {
                forms_1 = forms_1_1;
            },
            function (router_1_1) {
                router_1 = router_1_1;
            },
            function (articles_routes_1_1) {
                articles_routes_1 = articles_routes_1_1;
            },
            function (articles_component_1_1) {
                articles_component_1 = articles_component_1_1;
            },
            function (create_component_1_1) {
                create_component_1 = create_component_1_1;
            },
            function (create_component_2_1) {
                create_component_2 = create_component_2_1;
            },
            function (list_component_1_1) {
                list_component_1 = list_component_1_1;
            },
            function (list_component_2_1) {
                list_component_2 = list_component_2_1;
            },
            function (view_component_1_1) {
                view_component_1 = view_component_1_1;
            },
            function (edit_component_1_1) {
                edit_component_1 = edit_component_1_1;
            },
            function (view_component_2_1) {
                view_component_2 = view_component_2_1;
            },
            function (view_component_3_1) {
                view_component_3 = view_component_3_1;
            },
            function (create_component_3_1) {
                create_component_3 = create_component_3_1;
            },
            function (view_component_4_1) {
                view_component_4 = view_component_4_1;
            },
            function (view_component_5_1) {
                view_component_5 = view_component_5_1;
            },
            function (list_component_3_1) {
                list_component_3 = list_component_3_1;
            },
            function (create_component_4_1) {
                create_component_4 = create_component_4_1;
            }
        ],
        execute: function () {
            ArticlesModule = /** @class */ (function () {
                function ArticlesModule() {
                }
                ArticlesModule = __decorate([
                    core_1.NgModule({
                        imports: [
                            common_1.CommonModule,
                            forms_1.FormsModule,
                            router_1.RouterModule.forChild(articles_routes_1.ArticlesRoutes),
                        ],
                        declarations: [
                            articles_component_1.ArticlesComponent,
                            create_component_1.CreateComponentAlert,
                            create_component_3.CreateComponent,
                            list_component_1.ListComponent,
                            list_component_2.ListComponentAlert,
                            view_component_1.ViewComponent,
                            edit_component_1.EditComponent,
                            view_component_2.ViewByCourseComponent,
                            view_component_3.ViewByStudentComponent,
                            create_component_2.CreateComponentSymptom,
                            view_component_4.ViewBySymptomComponent,
                            view_component_5.ViewComponentVideo,
                            list_component_3.ListComponentTip,
                            create_component_4.CreateComponentTip
                        ]
                    })
                ], ArticlesModule);
                return ArticlesModule;
            }());
            exports_1("ArticlesModule", ArticlesModule);
        }
    };
});
//# sourceMappingURL=articles.module.js.map